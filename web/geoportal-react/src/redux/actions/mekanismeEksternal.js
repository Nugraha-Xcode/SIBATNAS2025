...
import Service from "src/services/mekanismeEksternal.service";
...
export const retrieve = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: RETRIEVE_MEKANISME_EKSTERNAL_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
...
export const create = (name) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: CREATE_MEKANISME_EKSTERNAL_SUCCESS,
      payload: res.data,
    });
...
export const update = (uuid, data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: UPDATE_MEKANISME_EKSTERNAL_SUCCESS,
      payload: data,
    });
...
export const remove = (uuid) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: DELETE_MEKANISME_EKSTERNAL_SUCCESS,
      payload: { uuid },
    });
  } catch (err) {
    if (err.response && err.response.status === 401) {
