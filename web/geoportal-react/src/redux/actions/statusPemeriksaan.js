...
import Service from "src/services/statusPemeriksaan.service";
...
export const retrieve = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: RETRIEVE_STATUS_PEMERIKSAAN_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
...
export const create = (name) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: CREATE_STATUS_PEMERIKSAAN_SUCCESS,
      payload: res.data,
    });
...
export const update = (uuid, data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: UPDATE_STATUS_PEMERIKSAAN_SUCCESS,
      payload: data,
    });
...
export const remove = (uuid) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: DELETE_STATUS_PEMERIKSAAN_SUCCESS,
      payload: { uuid },
    });
  } catch (err) {
    if (err.response && err.response.status === 401) {
