...
import Service from "src/services/eksternal.service";
...
export const retrieve = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: RETRIEVE_EKSTERNAL_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
...
export const retrieveBPKHTL = (uuid) => async (dispatch) => {
  try {
...
export const create =
  (name, akronim, kategoriEksternal, mekanismeEksternal) =>
  async (dispatch) => {
    try {
