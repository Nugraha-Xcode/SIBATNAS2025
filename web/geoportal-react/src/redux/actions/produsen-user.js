...
import Service from "src/services/produsen-user.service";
...
export const retrieve = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: RETRIEVE_PRODUSEN_USER_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
...
export const retrieveProdusenUser = (uuid) => async (dispatch) => {
  try {
...
export const create = (produsen, user) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: CREATE_PRODUSEN_USER_SUCCESS,
      payload: res.data,
    });
...
export const update = (uuid, data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: UPDATE_PRODUSEN_USER_SUCCESS,
      payload: data,
    });
...
export const remove = (data) => async (dispatch) => {
  try {
    const res = await Service.remove(data.produsen.uuid, data.user.uuid);
    let uuid = data.user.uuid;
    dispatch({
      type: DELETE_PRODUSEN_USER_SUCCESS,
      payload: { uuid },
    });
  } catch (err) {
    if (err.response && err.response.status === 401) {
