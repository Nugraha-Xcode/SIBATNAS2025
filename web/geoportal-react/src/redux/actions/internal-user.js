...
import Service from "src/services/internal-user.service";
...
export const retrieve = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: RETRIEVE_INTERNAL_USER_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
...
export const retrieveInternalUser = (uuid) => async (dispatch) => {
  try {
...
export const create = (internal, user) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: CREATE_INTERNAL_USER_SUCCESS,
      payload: res.data,
    });
...
export const update = (uuid, data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: UPDATE_INTERNAL_USER_SUCCESS,
      payload: data,
    });
...
export const remove = (data) => async (dispatch) => {
  try {
    const res = await Service.remove(data.internal.uuid, data.user.uuid);
    let uuid = data.user.uuid;
    dispatch({
      type: DELETE_INTERNAL_USER_SUCCESS,
      payload: { uuid },
    });
  } catch (err) {
    if (err.response && err.response.status === 401) {
