...
import { retrieve } from "src/redux/actions/metadata";
...
function MetadataTab() {
  ...
  const datas = useSelector((state) => state.metadata);
          ...
          <CardContent>
            ...
            <TableContainer>
              <Table width={300}>
                <TableHead>
                  <TableRow>
                    <TableCell>Nama File {label}</TableCell>
                    <TableCell>Waktu Buat</TableCell>
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_metadata}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
