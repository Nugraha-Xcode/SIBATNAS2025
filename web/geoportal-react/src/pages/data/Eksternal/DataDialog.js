...
import {
  retrieveIgtEksternal,
  retrieveIgtEksternalUser,
} from "src/redux/actions/igt-eksternal";

import { create, update, remove } from "src/redux/actions/dataEksternal";
...
function KategoriDialog(props) {
  ...
  const tematiks = useSelector((state) => state.igtEksternal);
  ...
  useEffect(() => {
    if (currentUser.roles.includes("ROLE_ADMIN")) {
      ...
      dispatch(retrieveIgtEksternalUser(currentUser.uuid));
    }
  }, []);
  ...
  useEffect(() => {
    if (config) {
      ...
      if (config.data) {
        setData(config.data);
        setSelectedIgt(config.data.igtEksternal.name);
      } else {
  ...
  const save = () => {
    ...
    dispatch(create(deskripsi, user, tematik, documentFile, dataSpasialFile))
      .then((data) => {
        //console.log(data);
