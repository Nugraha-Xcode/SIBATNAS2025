...
import { create, update, remove } from "src/redux/actions/dataPemeriksaan";
...
*/
    ...
    if (config) {
      ...
      if (config.data) {
        ...
        setData({
          ...
          dataProdusen: config.data.dataProdusen,
        });
  ...
  const save = (e) => {
    e.preventDefault();
    console.log(data);
    const { user, kategori, dataProdusen, uuid } = data;
    //console.log(data);
    ...
    if (isValidated()) {
      ...
      dispatch(create(us, kat, dataProdusen, uuid, documentFile))
        .then((data) => {
          console.log(data);
          //setSubmitted(true);
          setIsLoading(false);
