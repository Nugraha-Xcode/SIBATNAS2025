...
function UserTab() {
  ...
  const filterData = useCallback(() => {
    ...
    if (keyword) {
      ...
      result = result.filter(item => 
        ...
        (item.produsens && item.produsens.some(prod => 
          prod.name?.toLowerCase().includes(keyword.toLowerCase()) ||
          prod.akronim?.toLowerCase().includes(keyword.toLowerCase())
        ))
      );
    }
  ...
  useEffect(() => {
    ...
    dispatch(retrieveProdusenUser("0")).finally(() => {
      setLoading(false);
    });
  ...
  const handleProdusenChange = (e) => {
    ...
    if (value !== "Pilih Produsen Data Geospasial") {
      ...
      setProdusen(a[0]);
      setLoading(true);
      dispatch(retrieveProdusenUser(a[0].uuid)).finally(() => {
        setLoading(false);
      });
    } else {
      setSelectedProdusen(value);
  ...
  const handleClose = () => {
    ...
    if (produsen) {
      setLoading(true);
      dispatch(retrieveProdusenUser(produsen.uuid)).finally(() => {
        setLoading(false);
      });
    } else {
      setLoading(true);
        ...
        <Card>
          <CardContent>
            ...
            <Box sx={{ 
              ...
              <TextField
                placeholder="Cari user berdasarkan username, email, produsen..."
                variant="outlined"
                size="small"
                value={keyword}
              ...
              <Table>
                ...
                <TableBody>
                  {!loading && currentPageData.length === 0 ? (
                    <TableRow key={0}>
                      <TableCell colSpan={currentUser.roles.includes("ROLE_ADMIN") ? 5 : 4} align="center">
                        {selectedProdusen === "Pilih Produsen Data Geospasial"
                    ...
                    currentPageData.map((lokasi) => (
                      <TableRow key={lokasi.uuid} hover>
                        ...
                        <TableCell>{lokasi.email}</TableCell>
