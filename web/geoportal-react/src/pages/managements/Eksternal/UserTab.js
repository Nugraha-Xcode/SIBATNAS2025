...
import { retrieveEksternalUser } from "src/redux/actions/eksternal-user";
...
function UserTab() {
  ...
  const datas = useSelector((state) => state.eksternal_user);
  ...
  useEffect(() => {
    dispatch(retrieve());
    dispatch(retrieveEksternalUser("0"));
  }, []);
  ...
  const handleEksternalChange = (e) => {
    ...
    if (value !== "Pilih Eksternal") {
      ...
      setEksternal(a[0]);
      dispatch(retrieveEksternalUser(a[0].uuid));
    } else {
      setSelectedEksternal(value);
