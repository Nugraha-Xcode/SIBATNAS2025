...
import { create, remove, update } from "src/redux/actions/igt-eksternal";
...
function IgtDialog(props) {
  ...
  useEffect(() => {
    if (config) {
      ...
      if (config.data) {
        setData(config.data);
        setSelectedEksternal(config.data.eksternal?.name);
      } else {
  ...
  const save = (e) => {
    e.preventDefault();
    const { name, eksternal } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(name, eksternal))
        .then((data) => {
          //console.log(data);
          //setSubmitted(true);
          setLoading(false);
      ...
      <DialogContent>
        ...
        >
          ...
          <Select
            ...
            autoWidth
          >
            {eksternals.map((kategoriTematik) => (
