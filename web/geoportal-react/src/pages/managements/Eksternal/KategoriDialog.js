...
import { create, remove, update } from "src/redux/actions/kategoriEksternal";
import { retrieve } from "src/redux/actions/cakupanWilayah";
...
function KategoriDialog(props) {
  ...
  const cakupanWilayahs = useSelector((state) => state.cakupanWilayah);
  const initialDataState = {
    name: "",
    cakupanWilayah: cakupanWilayahs[0],
  };
  const [data, setData] = useState(initialDataState);
  // const [submitted, setSubmitted] = useState(false);
  const [selectedCakupan, setSelectedCakupan] = useState("");
  // const [submitted, setSubmitted] = useState(false);
  ...
  const handleCakupanChange = (e) => {
    ...
    var a = cakupanWilayahs.filter(function (el) {
      return el.name == value;
    });

    setData({ ...data, ["cakupanWilayah"]: a[0] });
  };
  ...
  useEffect(() => {
    if (config) {
      ...
      if (config.data) {
        setData(config.data);
        setSelectedCakupan(config.data.cakupanWilayah?.name);
      } else {
        setData(initialDataState);
        setSelectedCakupan(cakupanWilayahs[0]?.name);
      }
    }
  }, [config]);
  ...
  const save = (e) => {
    e.preventDefault();
    const { name, cakupanWilayah } = data;

    if (isValidated()) {
      setLoading(true);
      dispatch(create(name, cakupanWilayah))
        .then((data) => {
          //console.log(data);
          //setSubmitted(true);
          setLoading(false);
      ...
      <DialogContent>
        ...
        {currentUser.roles.includes("ROLE_ADMIN") ? (
          <FormControl fullWidth variant="outlined" sx={{ mt: 1 }}>
            <InputLabel>Cakupan Wilayah</InputLabel>
            <Select
            ...
            >
              {cakupanWilayahs.map((kategoriTematik) => (
                <MenuItem
