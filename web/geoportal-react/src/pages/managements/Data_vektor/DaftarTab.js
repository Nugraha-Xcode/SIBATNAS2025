...
import { retrieve } from "src/redux/actions/data_vektor";

import DaftarDialog from "./DaftarDialog";

function Data_vektorTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Data_vektor";
  ...
  const datas = useSelector((state) => state.data_vektor);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_data_vektor}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
