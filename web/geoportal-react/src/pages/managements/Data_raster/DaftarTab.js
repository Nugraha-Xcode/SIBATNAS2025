...
import { retrieve } from "src/redux/actions/data_raster";

import DaftarDialog from "./DaftarDialog";

function Data_rasterTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Data_raster";
  ...
  const datas = useSelector((state) => state.data_raster);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_data_raster}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
