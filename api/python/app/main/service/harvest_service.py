...
from app.main.model.records import Records
...
mix = 95.23925781250178
miy = -10.923083305360127
makx = 141.01939392089417
may = 5.65618228912069
bbox = Polygon([(mix,miy), (makx,miy), (makx,may), (mix, may), (mix, miy)])
        ...
        for rec in csw.records:
            ...
            if csw.records[rec].identifier:
                ...
                if csw.records[rec].type == 'dataset' :
                    ...
                    print(x4,y4)
                '''
                fminx = float(csw.records[rec].identification.bbox.minx)
                fminy = float(csw.records[rec].identification.bbox.miny)
                fmaxx = float(csw.records[rec].identification.bbox.maxx)
                fmaxy = float(csw.records[rec].identification.bbox.maxy)
                ...
                bbox2 = Polygon([(fminx,fminy),(fmaxx,fminy),(fmaxx,fmaxy),(fminx,fmaxy),(fminx,fminy)])
...
def get_harvest_by_bbox(minx, miny, maxx, maxy):
    if minx == -133044556.246885:
        ...
        print('SRID=4326;POLYGON(('+str(minx)+' '+str(miny)+','+str(maxx)+' '+str(miny)+','+str(maxx)+' '+str(maxy)+','+str(minx)+' '+str(maxy)+','+str(minx)+' '+str(miny)+'))')
...
def delete_harvested(data):
    harvesting = Harvestings.query.filter_by(identifier=data['identifier']).first()
    if harvesting:
        harvesting.query.filter_by(identifier=data['identifier']).delete()
        db.session.commit()
        #delete records by identifier
        records = Records.query.filter_by(identifier=data['identifier']).first()
        print(records)
        if records:
            records.query.filter_by(identifier=data['identifier']).delete()
            db.session.commit()
        
        #update flag status table metadata
        metadata = Metadata.query.filter_by(filename=data['filename']).first()
        if metadata:
            setattr(metadata, 'status',False)
            db.session.commit()
        response_object = {
            'status': 'success',
...
def get_count_all(query):
    #return Harvestings.query.order_by('title').all()
    if query != 'none':
        search = "%{}%".format(query)
        response_object = {
            'count': Harvestings.query.filter(Harvestings.title.ilike(search)).count(),
        }
        return response_object, 200
    else:
        response_object = {
...
def get_limit(order="title", page=0, page_size=20, query=None):
    #return Harvestings.query.order_by('title').all()
    ##limit(10).all()
    #query = Harvestings.query.order_by(order)
    #if page_size:
    #    query = Harvestings.query.order_by(order).limit(page_size).all()
    # if page:
    #query = Harvestings.query.order_by(order).limit(page_size).offset(page*page_size).all()
        #query = query.offset(page*page_size).all()
    #print(order, page, page_size)
    if query != 'none':
        search = "%{}%".format(query)
        return Harvestings.query.filter(Harvestings.title.ilike(search)).order_by(order).limit(page_size).offset(page*page_size).all()
    else:
        return Harvestings.query.order_by(order).limit(page_size).offset(page*page_size).all()
