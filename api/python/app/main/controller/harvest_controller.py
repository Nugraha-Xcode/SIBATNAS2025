...
@api.route('/limit/<string:order>/<int:page>/<int:page_size>/<string:query>')
...
class HarvestLimitPulls(Resource):
    ...
    @api.marshal_list_with(_schema2, envelope='data')
    # @token_required
    def get(self, order, page, page_size, query):
        """harvest an organization given its identifier"""
        return get_limit(order, page, page_size, query)
